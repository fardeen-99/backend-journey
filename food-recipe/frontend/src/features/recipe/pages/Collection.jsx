...
const navigate=useNavigate()
    ...
    console.log(recipe)
    return(
        <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-10 w-full p-6">
                 {recipe.map((item)=>{
                     return(
                        <section key={item._id} 
                        onClick={()=>navigate(`/collection/${item._id}`)}
                        className="group max-h-[300px] overflow-hidden relative flex items-center gap-2 justify-center flex-col w-full">
