...
const DetailPost = () => {
  ...
  return (
    <section className="w-full min-h-dvh md:h-dvh overflow-y-scroll flex flex-col md:flex-row items-center ">
      {singlepost?.post_url && (
        <>
          <div className=' w-full h-full flex flex-col md:w-[60%] bg-[#0C1014] ' key={singlepost._id}  >
            ...
            {/* <div className='flex justify-between w-full px-4 py-3 border-t border-zinc-800 text-white text-2xl'>
              <div className='flex gap-4 items-center'>
                <div className='flex gap-1 items-center cursor-pointer active:scale-95 transition-all'
                  onClick={() => {
                    singlepost.islike ? unlikeHandle(singlepost._id) : likeHandle(singlepost._id)
                    singlepostHandle() // refresh the local state
                  }}
                >
                  {singlepost.islike ? <FaHeart className='text-red-500' /> : <FaRegHeart />}
                  <span className='text-sm'>{singlepost.likecount > 0 ? singlepost.likecount : ""}</span>
                </div>
              </div>

              <div className='flex items-center cursor-pointer active:scale-95 transition-all'
                onClick={() => {
                  singlepost.save ? unsaveHandle(singlepost._id) : saveHandle(singlepost._id)
                  singlepostHandle() // refresh the local state
                }}
              >
                {singlepost.save ? <FaBookmark /> : <FaRegBookmark />}
              </div>
            </div> */}
